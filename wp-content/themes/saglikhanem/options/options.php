...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['footer_credit'] = __('', 'sociallyviral' );

		$defaults['help_tabs'] = array();
		$defaults['help_sidebar'] = __('', 'sociallyviral' );
	...
	*/
	function _enqueue(){
		...
		wp_localize_script( 'nhp-opts-js', 'nhpopts', array(
			...
			'reset_confirm' => __('Are you sure you want to reset ALL options to default?', 'MTSTHEMENAME' ),
			'import_confirm' => __('Are you sure you want to import options? All current options will be lost.', 'MTSTHEMENAME' ),
			'leave_page_confirm' => __('Settings have changed, you should save them! Are you sure you want to leave this page?', 'MTSTHEMENAME' ),
			'child_theme_name_empty' => __('Please enter desired child theme name.', 'MTSTHEMENAME' ),
			'import_done' => __('Importing proccess finished!', 'MTSTHEMENAME' ),
			'import_fail' => __('Importing proccess failed! Please try again.', 'MTSTHEMENAME' ),
			'remove_done' => __('Removal proccess finished!', 'MTSTHEMENAME' ),
			'remove_fail' => __('Removal proccess failed! Please try again.', 'MTSTHEMENAME' ),
			'reloading_page' => __('Reloading page...', 'MTSTHEMENAME' ),
			'import_opt_confirm' => __('Are you sure you want to import demo options? All current options will be lost.', 'MTSTHEMENAME' ),
			'import_all_confirm' => __('Are you sure you want to import demo?', 'MTSTHEMENAME' ),
			'remove_all_confirm' => __("Are you sure you want to remove demo options, content, menus and widgets? Please note that all modifications you've made to imported content will be lost", 'MTSTHEMENAME' ),
		) );
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							...
							if (!empty($this->args['presets'])) {
								...
								echo '<h4>'.__('Preset Options', 'MTSTHEMENAME' ).'</h4>';
								...
								foreach ( $this->args['presets'] as $key => $preset ) {
									echo '<div class="preset" data-nonce="'.$nonce.'" data-demo-id="'.esc_attr($key).'">';
										$imported_flag    = '<span class="demo_part_imported">'.__( '(imported)', 'MTSTHEMENAME' ).'</span>';
										...
										if ( ! empty( $preset['demo'] ) ) echo '<a href="'.$preset['demo'].'" target="_blank" class="demo-link">'.__('View demo', 'MTSTHEMENAME' ).'</a>';
										...
										echo '<div class="preset-actions-wrap">';
											echo '<button class="button button-secondary import-demo-options-button"'.$disabled_button.'>'.__('Import Theme Options', 'MTSTHEMENAME' ).'</button>';
											if ( !empty( $all_imported ) ) {
												echo '<button class="button button-primary remove-demo-button" data-nonce="'.$nonce.'">'.__('Remove imported data', 'MTSTHEMENAME' ).'</button>';
											} else {
												echo '<button class="button button-secondary import-demo-button"'.$disabled_button.'>'.__('Import Theme Options & Demo Content', 'MTSTHEMENAME' ).'</button>';
											}
										echo '</div>';
								...
								echo '<div id="importing-overlay" class="mfp-hide">';
									echo '<div id="importing-modal">';
										echo '<div id="importing-modal-header">';
											echo '<h2><span class="spinner is-active"></span>' . __( 'Processing, please wait...', 'MTSTHEMENAME' ) . '</h2>';
										echo '</div>';
										echo '<div id="importing-modal-content">';
										echo '</div>';
										echo '<div id="importing-modal-footer">';
											echo '<span id="importing-modal-footer-info">'.__( 'Processing, please wait...', 'MTSTHEMENAME' ).'</span>';
											echo '<button id="importing-modal-footer-button" class="button button-primary">'.__( 'Ok', 'MTSTHEMENAME' ).'</button>';
										echo '</div>';
									echo '</div>';
								echo '</div>';
