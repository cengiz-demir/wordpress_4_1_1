...
class mts_post_slider_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			sprintf( __('%sPost Slider', 'MTSTHEMENAME' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
			array( 'description' => __( 'Display posts from multiple categories in an animated slider.', 'MTSTHEMENAME' ) )
		);
	}

	public function form( $instance ) {
		$defaults = array(
			'title' => __( 'Featured Posts', 'MTSTHEMENAME' ),
			'cat' => array(),
			'slides_num' => 3,
			'show_title' => 0,
			'slider_nav' => 'bullets',
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'MTSTHEMENAME' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'cat' ); ?>"><?php _e( 'Category:', 'MTSTHEMENAME' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'slides_num' ); ?>"><?php _e( 'Number of Posts to show', 'MTSTHEMENAME' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'slider_nav' ); ?>"><?php _e( 'Navigaton type:', 'MTSTHEMENAME' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'slider_nav' ); ?>" name="<?php echo $this->get_field_name( 'slider_nav' ); ?>" class="widefat">
				<option value="bullets"<?php selected( $instance['slider_nav'], 'bullets' ); ?>><?php _e( 'Bulleted', 'MTSTHEMENAME' ); ?></option>
				<option value="arrows"<?php selected( $instance['slider_nav'], 'arrows' ); ?>><?php _e( 'Arrows', 'MTSTHEMENAME' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_title'); ?>">
				...
				<?php _e( 'Show Title', 'MTSTHEMENAME' ); ?>
			</label>
		</p>
					...
					<div class="widget-slider widget-slider-<?php echo $slider_nav; ?>">
						...
						<div class="slide">
							<a href="<?php echo esc_url( get_the_permalink() ); ?>">
								<?php the_post_thumbnail('sociallyviral-widgetfull',array('title' => '')); ?>
								<?php if ( $show_title ) { ?>
